...
public class Menu {
	...
	public void afficherNotification(Utilisateur u) {
		ArrayList<Notification> not = u.getNotifications();
		...
		// Menu pour revendeur
			...
			switch (option) {
				...
				case 5:
					...
					choix = prompt();
					if(choix == 1) afficherPageRevendeur();
			...
			switch (option) {
				case 1:
					...
					if(choix==(panier.getProduits().size()+1)){
						...
						System.out.println("Entrez [1] pour revenir au menu principal");
						// Lister tous les revendeurs dans le panier de l'acheteur
						ArrayList<Revendeur> listeRevendeur = new ArrayList<>();
						for(Produit p : util.getPanier().getProduits()){
							if(!listeRevendeur.contains(p.getRevendeur())) {
								listeRevendeur.add(p.getRevendeur());
							}
						}
						// Envoyer une nouvelle notifiacation aux revendeurs
						for(Revendeur r : listeRevendeur) {
							CategorieNotif categorieNotif = CategorieNotif.NOUVELLE_COMMANDE_RECUE;
							Notification notification = new Notification(CategorieNotif.NOUVELLE_COMMANDE_RECUE, categorieNotif.getDescription());
							r.notifier(notification);
						}
						choix=prompt();
				...
				case 6:
					...
					choix = prompt();
					if(choix == 1) afficherPageAcheteur();
					else System.out.println("Option indisponible");
					break;
				case 7:
					//metriques;
								...
								if(choix == prodAcht.size()) afficherPageAcheteur();
									...
									if (choix == 1) {
										...
										if(ajouterCom == 2) {
											...
											// Envoyer une notification au revendeur
											CategorieNotif categorieNotif = CategorieNotif.EVALUATION_PRODUIT;
											Notification notification = new Notification(CategorieNotif.EVALUATION_PRODUIT, categorieNotif.getDescription());
